...
function fetchHistoricalMetricsFromPostgres(component, metricName, hoursLimit = 24) {
  // Simulates SELECT timestamp, value FROM metrics WHERE component = $1 AND metric_name = $2 AND ...
  // We construct a mock database result matching actual metrics for PowerBI dashboards
  const dataset = [];
  const now = Date.now();
  ...
  if (metricName.includes('latency')) baseValue = 15;
