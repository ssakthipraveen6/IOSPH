...
const dbManager = {
  ...
  addMetric(component, metricName, value, env = null) {
    ...
    // Primary write path: TimescaleDB / Postgres async insert
    postgres.saveMetricToPostgres(entry).catch(() => {});
  ...
  addMetricBatch(metricsArray, env = null) {
    ...
    // Primary write path: TimescaleDB / Postgres multi-row batch insert
    postgres.saveMetricBatchToPostgres(metricsArray).catch(() => {});
