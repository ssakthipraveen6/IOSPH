...
function getRcaData(appKey) {
  ...
  const flows = {
    ...
    mcp: [
      { name: "SSO Gateway", value: "98ms latency", status: "Healthy" },
      { name: "AVI Load Balancer", value: aviStatus === 'Warning' ? "188.2 MB/s" : "45.2 MB/s", status: aviStatus },
      { name: "Backup Gateway Replicas", value: mcpStatus === 'Critical' ? "0/3 replicas active" : "3/3 replicas active", status: mcpStatus },
      { name: "Internal Microservices", value: mcpStatus === 'Critical' ? "Refused" : "Healthy", status: mcpStatus }
    ]
  };

  // 1. Locate selected app key mapping
  const selectedApp = appKey || 'jenkins';
  const compKey = appComponentMap[selectedApp] || 'jenkins_k8s';

  // Define target metric lookup
  let metricName = 'responseTime';
  if (compKey === 'artifactory' || compKey === 'argocd_k8s') {
    metricName = 'latency';
  } else if (compKey === 'fortify') {
    metricName = 'cpu';
  } else if (compKey === 'teamcity') {
    metricName = 'load';
  }

  // 2. Fetch telemetry from local JSONL DB
  const actualDBData = sqliteMetrics.queryHistoricalMetrics(compKey, metricName, 24);

  let timeline = [];
  if (actualDBData && actualDBData.length > 0) {
    timeline = actualDBData.slice(-12).map((item, idx) => {
      const dateObj = new Date(item.timestamp);
      const timeStr = `${String(dateObj.getHours()).padStart(2, '0')}:${String(dateObj.getMinutes()).padStart(2, '0')}`;
      const expectedVal = Math.max(10, Math.floor(item.value * 0.85));
      return {
        time: timeStr,
        expected: expectedVal,
        actual: parseFloat(item.value.toFixed(1)),
        nasIops: Math.floor(item.value * 3.5),
        note: item.value > (expectedVal * 1.6) ? "Anomaly Spike" : null
      };
    });
  } else {
    // Fallback static metrics
    timeline = [
      { time: "00:00", expected: 45, actual: 48, nasIops: 220 },
      { time: "02:00", expected: 45, actual: 47, nasIops: 210 },
      { time: "04:00", expected: 45, actual: 46, nasIops: 230 },
      { time: "06:00", expected: 50, actual: 52, nasIops: 280 },
      { time: "08:00", expected: 60, actual: 65, nasIops: 410 },
      { time: "10:00", expected: 75, actual: 195, nasIops: 1250, note: "Traffic Spike" },
      { time: "12:00", expected: 80, actual: 280, nasIops: 2450, note: "OOM Heap Spike" },
      { time: "14:00", expected: 75, actual: 95, nasIops: 820, note: "Auto-remediation run" },
      { time: "16:00", expected: 65, actual: 68, nasIops: 380 },
      { time: "18:00", expected: 55, actual: 58, nasIops: 310 },
      { time: "20:00", expected: 45, actual: 49, nasIops: 260 },
      { time: "22:00", expected: 45, actual: 47, nasIops: 240 }
    ];
  }
