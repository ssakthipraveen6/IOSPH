          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', maxHeight: '220px', overflowY: 'auto' }}>
            {recovery.length === 0 ? (
              ...
              [...recovery].reverse().map(rec => (
                <div key={rec.id} style={{ padding: '8px', background: 'var(--bg-dark)', borderRadius: '4px', borderLeft: `3px solid ${rec.status === 'Success' ? '#10b981' : rec.status === 'In-Progress' ? '#f59e0b' : '#ef4444'}` }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.75rem', fontWeight: 'bold' }}>
                    <span>{rec.action}</span>
                    <span style={{ color: rec.status === 'Success' ? '#10b981' : '#f59e0b' }}>{rec.status}</span>
                  </div>
                  <p style={{ fontSize: '0.7rem', color: 'var(--text-muted)', marginTop: '4px' }}>Target: {rec.component.toUpperCase()} | Reason: {rec.triggerReason}</p>
                </div>
              ))
            )}
          </div>
