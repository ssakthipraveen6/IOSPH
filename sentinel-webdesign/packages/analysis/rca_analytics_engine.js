...
const { SEVERITY, normalizeSeverity } = require('@sentinel/shared-constants');
...
function generateRcaFlows(selectedApp = 'jenkins', env = null) {
  ...
  APPS.forEach(app => {
    const isCritical = alerts.some(a => a.component && a.component.toLowerCase().includes(app) && normalizeSeverity(a.severity) === SEVERITY.CRITICAL);
    const isWarning = alerts.some(a => a.component && a.component.toLowerCase().includes(app) && normalizeSeverity(a.severity) === SEVERITY.WARNING);
    let appStatus = isCritical ? SEVERITY.CRITICAL : (isWarning ? SEVERITY.WARNING : SEVERITY.HEALTHY);
    ...
    if (targetEnv === 'prod' && !isCritical && !isWarning) {
      ...
      if (!rec || rec.length === 0 || rec.some(m => m.value === 'Data Not Available')) {
        appStatus = SEVERITY.DATA_UNAVAILABLE;
      }
    }
    ...
    const isProdOrStg = targetEnv === 'prod' || targetEnv === 'staging' || targetEnv === 'stg';
    if (isProdOrStg) {
      flows[app] = [
        {
      ...
      flows[app] = [
        {
          name: "Layer 1: SSO / eLDAP Gateway",
          value: "42ms Latency (99.9% Bind)",
          status: "Healthy"
        },
        {
          name: "Layer 2: AVI Ingress Balancer",
          value: "140 Conn/s (45.2 MB/s)",
          status: "Healthy"
        },
        {
          name: `Layer 3: ${app.toUpperCase()} Application Node`,
  ...
  if (targetEnv === 'demo') {
    ...
    // PROD & STAGING: Not collecting yet, return empty
    timeline = [];
  }
...
function getServiceNowTickets(env = null) {
  ...
  // PROD & STAGING: Telemetry not collected yet
  return [];
}
