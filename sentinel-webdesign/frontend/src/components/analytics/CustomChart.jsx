        ...
        <g transform={`translate(${paddingLeft + chartWidth + 20}, ${paddingTop})`}>
          {renderedLines.map((line, idx) => (
            <g key={idx} transform={`translate(0, ${idx * 20})`}>
              <rect x="0" y="2" width="12" height="12" fill={line.color} rx="3" />
              <text x="18" y="12" fill="var(--text-main)" fontSize="11" fontWeight="700" fontFamily="var(--font-sans)">
                {line.label}
              </text>
            </g>
          ))}
        ...
        {hoverIndex !== null && hoverDetails.length > 0 && (
          <g>
            ...
            >
              ...
              >
                <div style={{ fontSize: '10px', color: 'var(--text-muted)', fontWeight: 700, borderBottom: '1px solid var(--border-light)', paddingBottom: '4px', marginBottom: '6px' }}>
                  🕒 {hoverTime}
                </div>
                  ...
                  {hoverDetails.map((det, dIdx) => (
                    <div key={dIdx} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: '11px' }}>
                      <span style={{ display: 'flex', alignItems: 'center', gap: '5px', fontWeight: 600 }}>
                        <span style={{ width: '7px', height: '7px', borderRadius: '50%', backgroundColor: det.color, display: 'inline-block', boxShadow: `0 0 6px ${det.color}` }}></span>
                        {det.label}
                      </span>
                      <span style={{ fontFamily: 'var(--font-mono)', fontWeight: 800 }}>
                        {typeof det.value === 'number' && !isNaN(det.value) ? det.value.toFixed(1) : String(det.value || '0')} <span style={{ fontSize: '9px', fontWeight: 500, color: 'var(--text-muted)' }}>{unit}</span>
                      </span>
                    </div>
                  ))}
                </div>
