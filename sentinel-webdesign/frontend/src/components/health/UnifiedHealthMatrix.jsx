...
export default function UnifiedHealthMatrix({ 
  ...
  const getStatusText = (status) => {
    if (status === 'DATA_UNAVAILABLE' || status === 'NO_DATA') return 'Value Not Available';
    return status || 'Healthy';
  };
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2rem' }}>
        ...
        <div className="console-panel" style={{ padding: '1.5rem' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', marginTop: '0.5rem' }}>
            {appItems.map(app => {
              const status = isProdOrStg ? 'DATA_UNAVAILABLE' : (componentStatuses[app.key] || 'Healthy');
              const activeAlertsCount = isProdOrStg ? 0 : alerts.filter(a => a.component === app.key && a.status === 'Active').length;
              ...
              return (
                <div 
                  ...
                  onClick={() => {
                    if (onInspectEntity) onInspectEntity(app.key);
                    else if (onSelectComponent) onSelectComponent(app.key);
                  }}
                  style={{ 
                ...
                >
                  <div>
                    <span style={{ fontWeight: 800, fontSize: '0.92rem', display: 'block', color: 'var(--text-main)' }}>
                      {app.name}
                    </span>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                    {activeAlertsCount > 0 && (
                      <span style={{ fontSize: '0.65rem', backgroundColor: 'var(--critical-glass)', color: 'var(--critical)', border: '1px solid var(--critical)', padding: '2px 8px', borderRadius: '4px', fontWeight: 800 }}>
        ...
        <div className="console-panel" style={{ padding: '1.5rem' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', marginTop: '0.5rem' }}>
            {infraItems.map(infra => {
              const status = isProdOrStg ? 'DATA_UNAVAILABLE' : (componentStatuses[infra.key] || 'Healthy');
              const activeAlertsCount = isProdOrStg ? 0 : alerts.filter(a => a.component === infra.key && a.status === 'Active').length;
              ...
              return (
                <div 
                  ...
                  className={`app-status-row ${getStatusClass(status)}`}
                  onClick={() => {
                    if (onSelectComponent) onSelectComponent(infra.key);
                  }}
                  style={{ 
                    borderLeft: `4px solid ${getStatusDot(status)}`,
                    padding: '14px 16px',
      ...
      <div className="console-panel" style={{ padding: '1.5rem' }}>
        ...
        <div className="table-wrapper" style={{ marginTop: '0.75rem', maxHeight: 'none' }}>
          <table className="telemetry-table" style={{ width: '100%' }}>
            ...
            <tbody>
              {isProdOrStg || customChecks.length === 0 ? (
                <tr>
                  <td colSpan="7" style={{ textAlign: 'center', color: 'var(--text-muted)', padding: '16px' }}>
                    {isProdOrStg ? `Value Not Available (Custom check telemetry not collected for ${currentEnv.toUpperCase()})` : 'No custom checks loaded from backend server.'}
                  </td>
                </tr>
              ) : (
