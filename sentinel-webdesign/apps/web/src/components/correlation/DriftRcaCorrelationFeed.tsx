...
export interface DriftLinkedIncident {
  incidentId: string;
  service: string;
  priority: string;
...
export interface DriftRcaStats {
  headlineStat: string;
  timeWindowMinutes: number;
  samplePeriod: string;
  driftPrecededIncidentsCount: number;
  totalIncidentsAnalyzed: number;
  driftLinkedIncidents: DriftLinkedIncident[];
}

export interface DriftRcaCorrelationFeedProps {
  onSelectEntity?: (entityId: string) => void;
  onNavigateToTimeline?: (entityId: string) => void;
}

export default function DriftRcaCorrelationFeed({
  onSelectEntity,
  onNavigateToTimeline
}: DriftRcaCorrelationFeedProps): React.JSX.Element {
  const stats = DRIFT_RCA_CORRELATION_STATS as DriftRcaStats;
    ...
    <div className="drift-rca-correlation-feed" style={{ marginTop: '2rem', animation: 'fadeIn 0.2s ease-out' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          {stats.driftLinkedIncidents.map((item: DriftLinkedIncident) => (
            <div
              key={item.incidentId}
              style={{
                padding: '12px 14px',
