...
export default function MetricsDetail({ selectedComponent, onComponentChange, historicalMetrics, healthData = {}, environment = 'staging' }) {
  ...
  const [telemetryTab, setTelemetryTab] = useState('live'); // 'live' | 'historical'
  ...
  const isUnavailableVal = (val) => {
    return val === null || val === undefined || val === 'Data Not Available' || val === 'Value Not Available';
  };
  ...
  const getLatestMetricValue = (metricsArray, metricName, defaultValue) => {
    if (currentEnv === 'prod' || currentEnv === 'staging') {
      if (!metricsArray || metricsArray.length === 0) return 'Value Not Available';
      const matching = metricsArray.filter(m => m.metricName === metricName);
      if (matching.length === 0) return 'Value Not Available';
      const val = matching[matching.length - 1].value;
      if (isUnavailableVal(val)) return 'Value Not Available';
      return val;
    }
    ...
    if (matching.length > 0) {
      const val = matching[matching.length - 1].value;
      if (isUnavailableVal(val)) return defaultValue;
      return val;
    }
    return defaultValue;
  };
  ...
  const fmtVal = (val, unit, dec = 1) => {
    if (isUnavailableVal(val) || isNaN(parseFloat(val))) {
  ...
  const fmtInt = (val, unit) => {
    if (isUnavailableVal(val) || isNaN(parseInt(val))) {
      return 'Value Not Available';
    }
  ...
  const getActiveMetrics = (comp = selectedComponent) => {
    ...
    if (rawData && rawData.length > 0) {
      // [FIX-4] Strip sentinel unavailable records written by db.js for empty components
      const realData = rawData.filter(
        m => m.value !== 'Data Not Available' && m.value !== 'Value Not Available' && m.metricName !== 'status'
      );
    ...
    const infraConfigs = {
      sso_gateway: {
        ...
        getRowData: (app) => {
          ...
          if (currentEnv === 'prod' || currentEnv === 'staging') {
            if (!isUnavailableVal(rawSessions) && !isNaN(parseInt(rawSessions))) {
              displaySessions = `${parseInt(rawSessions)} sessions`;
            } else {
              displaySessions = 'Value Not Available';
            }

            if (!isUnavailableVal(rawLatency) && !isNaN(parseFloat(rawLatency))) {
              displayLatency = `${parseFloat(rawLatency).toFixed(0)}ms`;
            } else {
              displayLatency = 'Value Not Available';
            }

            if (displaySessions === 'Value Not Available' && displayLatency === 'Value Not Available') {
      ...
      avi_load_balancer: {
        ...
        getRowData: (app) => {
          ...
          const isUnavail = (currentEnv === 'prod' || currentEnv === 'staging') && (isUnavailableVal(flow) || isUnavailableVal(lat));
          return {
            col1: `vip-${app.key}.internal.corp`,
            col2: app.port || '443',
            col3: fmtVal(flow, 'MB/s', 1),
            col4: fmtVal(lat, 'ms', 0),
            col5: isUnavail ? 'Value Not Available' : 'Valid (TLS 1.3)',
            statusText: isUnavail ? 'Value Not Available' : 'UP / HEALTHY',
      ...
      database: {
        ...
        getRowData: (app) => {
          ...
          const isUnavail = (currentEnv === 'prod' || currentEnv === 'staging') && (isUnavailableVal(conns) || isUnavailableVal(tps));
          return {
            ...
            statusText: isUnavail ? 'Value Not Available' : (isDb ? 'ACTIVE' : 'INACTIVE'),
      ...
      linux_servers: {
        ...
        getRowData: (app) => {
          ...
          const isUnavail = (currentEnv === 'prod' || currentEnv === 'staging') && (isUnavailableVal(cpu) || isUnavailableVal(mem));
          return {
            col1: `${app.key}-linux-node.internal.corp`,
            col2: app.server || 'RHEL VM',
            col3: fmtVal(cpu, '%', 1),
            col4: fmtVal(mem, 'GB', 1),
            col5: isUnavail ? 'Value Not Available' : '142 threads',
            statusText: isUnavail ? 'Value Not Available' : 'ONLINE',
            statusClass: isUnavail ? 'status-unknown' : 'status-healthy'
          };
        }
      },
      windows_servers: {
        ...
        getRowData: (app) => {
          ...
          return {
            ...
            col3: fmtVal(cpu, '%', 1),
            col4: fmtVal(mem, 'GB', 1),
            col5: isUnavail ? 'Value Not Available' : (isWinApp ? '1420 active' : 'N/A'),
            statusText: isUnavail ? 'Value Not Available' : 'ONLINE',
            statusClass: isUnavail ? 'status-unknown' : 'status-healthy'
          };
        }
      ...
      nas_performance: {
        ...
        getRowData: (app) => {
          ...
          const isUnavail = (currentEnv === 'prod' || currentEnv === 'staging') && (isUnavailableVal(space) || isUnavailableVal(iops));
          return {
            col1: `d:\\production_shares\\nas_logs\\${app.key}`,
            col2: 'NFS v4.1',
            col3: fmtVal(space, 'GB', 1),
            col4: fmtInt(iops, 'IOPS'),
            col5: isUnavail ? 'Value Not Available' : '45 MB/s',
            statusText: isUnavail ? 'Value Not Available' : (hasNas ? 'MOUNTED' : 'UNMOUNTED'),
      ...
      s3_storage: {
        ...
        getRowData: (app) => {
          ...
          const isUnavail = (currentEnv === 'prod' || currentEnv === 'staging') && (isUnavailableVal(space) || isUnavailableVal(bw));
          return {
            col1: `s3://prod-${app.key}-telemetry-bucket`,
            col2: 'S3 Standard',
            col3: fmtInt(space, 'GB'),
            col4: fmtVal(bw, 'MB/s', 1),
            col5: isUnavail ? 'Value Not Available' : '14.5 ms',
            statusText: isUnavail ? 'Value Not Available' : 'SYNCED',
            statusClass: isUnavail ? 'status-unknown' : 'status-healthy'
          };
        }
      },
      network_latency: {
        ...
        getRowData: (app) => {
          ...
          if ((currentEnv === 'prod' || currentEnv === 'staging') && (isUnavailableVal(rawLat) || typeof rawLat !== 'number')) {
            return {
              col1: app.host || `${app.key}-prod.internal.corp`,
              col2: 'Value Not Available',
              col3: 'Value Not Available',
              col4: 'Value Not Available',
              col5: 'Value Not Available',
              statusText: 'Value Not Available',
              statusClass: 'status-unknown'
            };
          }
        ...
        <div className="console-panel" style={{ padding: '1.25rem' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '12px' }}>
            ...
            <div style={{ backgroundColor: 'var(--bg-dark)', border: '1px solid var(--border-light)', borderRadius: '6px', padding: '8px', textAlign: 'center' }}>
              ...
              <span className={`status-badge-inline ${status === 'DATA_UNAVAILABLE' ? 'status-unknown' : 'status-healthy'}`} style={{ fontSize: '0.65rem', marginTop: '6px' }}>{status === 'DATA_UNAVAILABLE' ? 'Value Not Available' : 'ONLINE'}</span>
            </div>
    ...
    if (metricNames.length === 0) {
      return (
        <div className="empty-chart" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '120px' }}>
          <p style={{ color: 'var(--text-muted)', fontSize: '0.75rem' }}>Value Not Available</p>
        </div>
      );
    }
  ...
  return (
    <div className="metrics-detail-container" style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      ...
      {/* Telemetry Sub-Tab Navigation */}
      <div style={{ display: 'flex', gap: '10px', borderBottom: '1px solid var(--border-light)', paddingBottom: '12px' }}>
        <button
          id="btn-telemetry-live"
          onClick={() => setTelemetryTab('live')}
          style={{
            padding: '8px 18px',
            background: telemetryTab === 'live' ? 'rgba(59, 130, 246, 0.15)' : 'transparent',
            border: telemetryTab === 'live' ? '1px solid var(--primary)' : '1px solid var(--border-light)',
            color: telemetryTab === 'live' ? 'var(--primary)' : 'var(--text-muted)',
            borderRadius: '6px',
            fontWeight: 600,
            fontSize: '0.85rem',
            cursor: 'pointer',
        ...
        >
          <span>📈</span> Live Golden Signals & Timeseries (1m Refresh)
        </button>
        <button
          id="btn-telemetry-historical"
          onClick={() => setTelemetryTab('historical')}
          style={{
            padding: '8px 18px',
            background: telemetryTab === 'historical' ? 'rgba(59, 130, 246, 0.15)' : 'transparent',
            border: telemetryTab === 'historical' ? '1px solid var(--primary)' : '1px solid var(--border-light)',
            color: telemetryTab === 'historical' ? 'var(--primary)' : 'var(--text-muted)',
            borderRadius: '6px',
            fontWeight: 600,
            fontSize: '0.85rem',
            cursor: 'pointer',
        ...
        >
          <span>📊</span> Historical Baseline Trends & PowerBI Analytics (1H - 90D)
        </button>
      </div>

      {telemetryTab === 'historical' ? (
        <PowerBiDashboard environment={currentEnv} />
      ) : (
        <>
      {/* Selector Ribbon */}
              ...
              <div className="console-panel" style={{ padding: '1rem' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', flex: 1 }}>
                  ...
                  <div style={{ backgroundColor: 'var(--bg-dark)', border: '1px solid var(--border-light)', borderRadius: '6px', padding: '6px', textAlign: 'center' }}>
                    ...
                    <span style={{ fontSize: status === 'DATA_UNAVAILABLE' ? '0.72rem' : '1.1rem', fontWeight: 700, color: status === 'Healthy' ? '#10b981' : (status === 'DATA_UNAVAILABLE' ? '#94a3b8' : '#ef4444') }}>
                      {status === 'DATA_UNAVAILABLE' ? 'Value Not Available' : (status === 'Healthy' ? '0' : '1')}
                    </span>
                  </div>
                  <div style={{ backgroundColor: 'var(--bg-dark)', border: '1px solid var(--border-light)', borderRadius: '6px', padding: '6px', textAlign: 'center' }}>
                    <span style={{ fontSize: '0.6rem', textTransform: 'uppercase', color: '#64748b', display: 'block', fontWeight: 600 }}>Node Status</span>
                    <span style={{ fontSize: '0.75rem', fontWeight: 700, color: status === 'DATA_UNAVAILABLE' ? '#94a3b8' : 'var(--primary)', textTransform: 'uppercase' }}>
                      {status === 'DATA_UNAVAILABLE' ? 'Value Not Available' : 'ONLINE'}
                    </span>
                  </div>
                  <div style={{ backgroundColor: 'var(--bg-dark)', border: '1px solid var(--border-light)', borderRadius: '6px', padding: '6px', textAlign: 'center' }}>
                    <span style={{ fontSize: '0.6rem', textTransform: 'uppercase', color: '#64748b', display: 'block', fontWeight: 600 }}>Health Rating</span>
                    <span style={{ fontSize: '0.75rem', fontWeight: 700, color: status === 'DATA_UNAVAILABLE' ? '#94a3b8' : 'var(--text-main)' }}>
                      {status === 'DATA_UNAVAILABLE' ? 'Value Not Available' : (status === 'Healthy' ? '100% Stable' : 'Degraded')}
                    </span>
                  </div>
                </div>
              </div>
                  ...
                  <table className="telemetry-table" style={{ width: '100%' }}>
                    ...
                    <tbody>
                      {(() => {
                        ...
                        const pgHasData = (historicalMetrics?.['database'] || []).some(m => (!m.env || m.env === currentEnv) && m.value !== 'Data Not Available' && m.value !== 'Value Not Available' && m.value !== null);
                        const nasHasData = (historicalMetrics?.['nas_performance'] || []).some(m => (!m.env || m.env === currentEnv) && m.value !== 'Data Not Available' && m.value !== 'Value Not Available' && m.value !== null);
                        const aviHasData = (historicalMetrics?.['avi_load_balancer'] || []).some(m => (!m.env || m.env === currentEnv) && m.value !== 'Data Not Available' && m.value !== 'Value Not Available' && m.value !== null);
                        const winHasData = (historicalMetrics?.['windows_servers'] || []).some(m => (!m.env || m.env === currentEnv) && m.value !== 'Data Not Available' && m.value !== 'Value Not Available' && m.value !== null);
                        ...
                        return (
                          <>
                            <tr>
                              ...
                              <td style={{ fontFamily: 'monospace', fontWeight: 'bold', color: !isProdOrStg || pgHasData ? '#ef4444' : 'var(--text-muted)' }}>
                                {!isProdOrStg || pgHasData ? '94.2%' : 'Value Not Available'}
                              </td>
                              <td>
                                {!isProdOrStg || pgHasData ? (
                                  <span className="status-pill status-critical" style={{ fontSize: '0.65rem' }}>CRITICAL</span>
                                ) : (
                                  <span className="status-pill status-unknown" style={{ fontSize: '0.65rem' }}>Value Not Available</span>
                                )}
                              </td>
                            </tr>
                            <tr>
                              ...
                              <td style={{ fontFamily: 'monospace', fontWeight: 'bold', color: !isProdOrStg || nasHasData ? '#ef4444' : 'var(--text-muted)' }}>
                                {!isProdOrStg || nasHasData ? '89.5%' : 'Value Not Available'}
                              </td>
                              <td>
                                {!isProdOrStg || nasHasData ? (
                                  <span className="status-pill status-critical" style={{ fontSize: '0.65rem' }}>CRITICAL</span>
                                ) : (
                                  <span className="status-pill status-unknown" style={{ fontSize: '0.65rem' }}>Value Not Available</span>
                                )}
                            ...
                            <tr>
                              ...
                              <td style={{ fontFamily: 'monospace', fontWeight: 'bold', color: !isProdOrStg || aviHasData ? '#f59e0b' : 'var(--text-muted)' }}>
                                {!isProdOrStg || aviHasData ? '76.1%' : 'Value Not Available'}
                              </td>
                              <td>
                                {!isProdOrStg || aviHasData ? (
                                  <span className="status-pill status-warning" style={{ fontSize: '0.65rem' }}>WARNING</span>
                                ) : (
                                  <span className="status-pill status-unknown" style={{ fontSize: '0.65rem' }}>Value Not Available</span>
                                )}
                              </td>
                            ...
                            <tr>
                              ...
                              <td style={{ fontFamily: 'monospace', fontWeight: 'bold', color: !isProdOrStg || winHasData ? '#10b981' : 'var(--text-muted)' }}>
                                {!isProdOrStg || winHasData ? '52.4%' : 'Value Not Available'}
                              </td>
                              <td>
                                {!isProdOrStg || winHasData ? (
                                  <span className="status-pill status-healthy" style={{ fontSize: '0.65rem' }}>STABLE</span>
                                ) : (
                                  <span className="status-pill status-unknown" style={{ fontSize: '0.65rem' }}>Value Not Available</span>
                                )}
                              </td>
