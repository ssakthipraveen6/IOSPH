...
export interface SparklineComputedData {
...
export interface SparklineProps {
  data?: (number | { value?: number; v?: number } | string)[];
  width?: number;
  height?: number;
  color?: string;
  strokeWidth?: number;
...
export default function Sparkline({
  ...
  const pointsData = useMemo<SparklineComputedData | null>(() => {
    if (!Array.isArray(data) || data.length === 0) {
      return null;
    }

    const numericValues: number[] = data.map(d => {
      if (typeof d === 'number') return d;
      if (d && typeof d === 'object') {
        if (typeof d.value === 'number') return d.value;
        if (typeof d.v === 'number') return d.v;
      }
    ...
    const coords: SparklinePoint[] = numericValues.map((val, idx) => {
