...
### In-Scope Assets
- **Application Services**: `apps/api` (REST endpoints, WebSocket broadcasts, JWT authentication).
- **Frontend Portal**: `apps/web` (React Single-Page Application, session state management).
- **Telemetry Ingestion**: `apps/collector` (OTLP receivers, Prometheus scrapers, Syslog/Fluentd streams).
- **Core Packages**: `packages/database`, `packages/logger`, `packages/config`, `packages/shared-constants`.
- **Credential Providers**: CyberArk Central Credential Provider (CCP) and Vault integrations in `packages/config/cyberark/`.
...
- **Vault Scoping**: Credentials retrieved via `packages/config/cyberark/credential_provider.js` are cached strictly in-memory with automatic TTL expiration (300s) to prevent memory persistence.
- **Audit Logging**: Any administrative override or authentication failure is recorded immutably in `apps/api/logs/sentinel_audit.log`.
