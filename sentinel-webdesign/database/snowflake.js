...
/**
 * [DEPRECATED / UNINVOKED] Simulated bulk logs exporter to Snowflake.
 * Note: Sentinel log writes stream directly to NAS/disk/SIEM; this function is uninvoked.
 * Marked deprecated to resolve architectural audit finding 1.3.
 */
function saveLogsToSnowflake(logsArray, writeNasLog) {
  if (writeNasLog) {
    writeNasLog('WARN', 'SNOWFLAKE', 'saveLogsToSnowflake invoked but bulk ingest is handled via external pipeline.');
  }
}

/**
 * [ACTIVE] Query endpoint for historical log aggregation.
 * Invoked by: apps/api/src/server.js via GET /api/pbi/logs for PowerBI/Historical Telemetry view.
 */
