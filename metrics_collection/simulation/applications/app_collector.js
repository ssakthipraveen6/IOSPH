...
function collectAppMetrics(simulations, db, writeNasLog) {
  ...
  appsList.forEach(({ key, collector }) => {
    ...
    // Check simulated outage override
    if (simulations[key] && simulations[key].type === 'outage') {
      state = 'Critical';
    } else if (key === 'artifactory' && simulations['artifactory']) {
      const vol = simulations['artifactory'].leakVolume || 50;
      state = vol > 90 ? 'Critical' : vol > 75 ? 'Warning' : 'Healthy';
    }
    ...
    writeNasLog('INFO', 'APP_SIMULATOR', `${key} | Status: ${state} | Metrics: ${metricsStr}`);
  });
